...
open class LineChart: UIView {
    ...
    open var cx: Coordinate = Coordinate()
    open var cy: Coordinate = Coordinate()
    ...
    fileprivate var drawingHeight: CGFloat = 0 {
        didSet {
            ...
            cy.linear = LinearScale(domain: [min, max], range: [0, drawingHeight])
            cy.scale = cy.linear.scale()
            cy.ticks = cy.linear.ticks(Int(cy.grid.count))
        }
    }
    fileprivate var drawingWidth: CGFloat = 0 {
        didSet {
            let data = dataStore[0]
            cx.linear = LinearScale(domain: [0.0, CGFloat(data.count - 1)], range: [0, drawingWidth])
            cx.scale = cx.linear.scale()
            cx.invert = cx.linear.invert()
            cx.ticks = cx.linear.ticks(Int(cx.grid.count))
        }
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        self.drawingHeight = self.bounds.height - (2 * cy.axis.inset)
        self.drawingWidth = self.bounds.width - (2 * cx.axis.inset)
        ...
        // draw grid
        if cx.grid.visible && cy.grid.visible { drawGrid() }
        
        // draw axes
        if cx.axis.visible && cy.axis.visible { drawAxes() }
        
        // draw labels
        if cx.labels.visible { drawXLabels() }
        if cy.labels.visible { drawYLabels() }
    ...
    fileprivate func handleTouchEvents(_ touches: NSSet!, event: UIEvent) {
        ...
        let inverted = self.cx.invert(xValue - cx.axis.inset)
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            let xValue = self.cx.scale(CGFloat(index)) + cx.axis.inset - dots.outerRadius/2
            let yValue = self.bounds.height - self.cy.scale(data[index]) - cy.axis.inset - dots.outerRadius/2
    ...
    fileprivate func drawAxes() {
        ...
        // draw x-axis
        cx.axis.color.setStroke()
        let y0 = height - self.cy.scale(0) - cy.axis.inset
        path.move(to: CGPoint(x: cx.axis.inset, y: y0))
        path.addLine(to: CGPoint(x: width - cx.axis.inset, y: y0))
        path.stroke()
        // draw y-axis
        cy.axis.color.setStroke()
        path.move(to: CGPoint(x: cx.axis.inset, y: height - cy.axis.inset))
        path.addLine(to: CGPoint(x: cx.axis.inset, y: cy.axis.inset))
        path.stroke()
    }
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        var xValue = self.cx.scale(0) + cx.axis.inset
        var yValue = self.bounds.height - self.cy.scale(data[0]) - cy.axis.inset
        path.move(to: CGPoint(x: xValue, y: yValue))
        for index in 1..<data.count {
            xValue = self.cx.scale(CGFloat(index)) + cx.axis.inset
            yValue = self.bounds.height - self.cy.scale(data[index]) - cy.axis.inset
    ...
    fileprivate func drawAreaBeneathLineChart(_ lineIndex: Int) {
        ...
        // move to origin
        path.move(to: CGPoint(x: cx.axis.inset, y: self.bounds.height - self.cy.scale(0) - cy.axis.inset))
        // add line to first data point
        path.addLine(to: CGPoint(x: cx.axis.inset, y: self.bounds.height - self.cy.scale(data[0]) - cy.axis.inset))
        // draw whole line chart
        for index in 1..<data.count {
            let x1 = self.cx.scale(CGFloat(index)) + cx.axis.inset
            let y1 = self.bounds.height - self.cy.scale(data[index]) - cy.axis.inset
            path.addLine(to: CGPoint(x: x1, y: y1))
        }
        // move down to x axis
        path.addLine(to: CGPoint(x: self.cx.scale(CGFloat(data.count - 1)) + cx.axis.inset, y: self.bounds.height - self.cy.scale(0) - cy.axis.inset))
        // move to origin
        path.addLine(to: CGPoint(x: cx.axis.inset, y: self.bounds.height - self.cy.scale(0) - cy.axis.inset))
        path.fill()
    }
    ...
    fileprivate func drawXGrid() {
        cx.grid.color.setStroke()
        let path = UIBezierPath()
        var x1: CGFloat
        let y1: CGFloat = self.bounds.height - cy.axis.inset
        let y2: CGFloat = cy.axis.inset
        let (start, stop, step) = self.cx.ticks
        for i in stride(from: start, through: stop, by: step){
            x1 = self.cx.scale(i) + cx.axis.inset
    ...
    fileprivate func drawYGrid() {
        self.cy.grid.color.setStroke()
        let path = UIBezierPath()
        let x1: CGFloat = cx.axis.inset
        let x2: CGFloat = self.bounds.width - cx.axis.inset
        var y1: CGFloat
        let (start, stop, step) = self.cy.ticks
        for i in stride(from: start, through: stop, by: step){
            y1 = self.bounds.height - self.cy.scale(i) - cy.axis.inset
            path.move(to: CGPoint(x: x1, y: y1))
    ...
    fileprivate func drawXLabels() {
        ...
        let y = self.bounds.height - cx.axis.inset
        let (_, _, step) = cx.linear.ticks(xAxisData.count)
        let width = cx.scale(step)
        
        var text: String
        for (index, _) in xAxisData.enumerated() {
            let xValue = self.cx.scale(CGFloat(index)) + cx.axis.inset - (width / 2)
            let label = UILabel(frame: CGRect(x: xValue, y: y, width: width, height: cx.axis.inset))
            label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
            label.textAlignment = .center
            if (cx.labels.values.count != 0) {
                text = cx.labels.values[index]
            } else {
                text = String(index)
            }
            label.text = text
    ...
    fileprivate func drawYLabels() {
        ...
        let (start, stop, step) = self.cy.ticks
        for i in stride(from: start, through: stop, by: step){
            yValue = self.bounds.height - self.cy.scale(i) - (cy.axis.inset * 1.5)
            let label = UILabel(frame: CGRect(x: 0-20, y: yValue, width: cy.axis.inset+20, height: cy.axis.inset))
            label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
            label.textAlignment = .center
