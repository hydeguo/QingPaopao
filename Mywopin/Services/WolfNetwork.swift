...
public class WolfModel {
    ...
    public static func modelList<T: WolfMapper>(_ data: Data?) -> [T]? {
        if let data = data {
            do {
                return try JSONDecoder.init().decode([T].self, from: data)
            } catch {
...
public class Wolf {
    ...
    ///   - failure:    失败回调
    /// - Returns:      Cancellable
    public class func request<T: WolfMapper, R: TargetType>(type: R, progress: ProgressBlock? = nil, completion: ((T?, String?, String?) -> ())?, failure: ((MoyaError?) -> ())?) -> Cancellable {
...
//                Log("type:\(type) |response:\(String(data: response.data, encoding: String.Encoding(rawValue: String.Encoding.utf8.rawValue)))")
    ...
    public class func requestBase<T: WolfMapper, R: TargetType>(type: R, progress: ProgressBlock? = nil, completion: ((T?, String?, String?) -> ())?, failure: ((MoyaError?) -> ())?) -> Cancellable {
...
//                Log("type:\(type) |response:\(response)")
    ...
    ///   - failure:    失败回调
    /// - Returns:      Cancellable
    public class func requestList<T: WolfMapper, R: TargetType>(type: R, progress: ProgressBlock? = nil, completion: (([T]?, String?, String?) -> Void)?, failure: ((MoyaError?) -> ())?) -> Cancellable {
        let provider = self.createProvider(type)
        return provider.request(type, progress: progress, completion: { (event) in
            switch event {
            case let .success(response):
//                Log("type:\(type) |response:\(String(data: response.data, encoding: String.Encoding(rawValue: String.Encoding.utf8.rawValue)))")
    ...
    public class func requestBaseList<T: WolfMapper, R: TargetType>(type: R, progress: ProgressBlock? = nil, completion: (([T]?, String?, String?) -> Void)?, failure: ((MoyaError?) -> ())?) -> Cancellable {
        let provider = self.createProvider(type)
        return provider.request(type, progress: progress, completion: { (event) in
            switch event {
            case let .success(response):
//                Log(response)
                WolfTransformModel.baseListFromJSON(response.data, completion)
            case let .failure(error):
                debugPrint(error.localizedDescription)
                failure?(error)
            }
...
//            return MoyaProvider<T>(endpointClosure: endpointClosure, manager: wolf.sessionManager,plugins: [CredentialsPlugin { target -> URLCredential? in
//                switch target {
//                default:
//                    Log(target)
//                    return URLCredential(user: wooConsumerWriteKey, password: wooConsumerWriteSecret, persistence: .none)
//                }
//                }
//            ])
        }
...
fileprivate struct WolfTransformModel {
    ...
    static func objectFromJSON<T: WolfMapper>(_ Json: Data, _ response: ((T?, String?, String?) -> Void)?) {
        let tmp: WolfBaseModel<T>? = WolfModel.model(Json)
        response?(tmp?.result, tmp?.msg, tmp?.status)
    }
    
    static func listFromJSON<T: WolfMapper>(_ Json: Data, _ response: (([T]?, String?, String?) -> Void)?) {
        let tmp: WolfBaseModels<T>? = WolfModel.model(Json)
        response?(tmp?.result, tmp?.msg, tmp?.status)
    }
    static func baseObjectFromJSON<T: WolfMapper>(_ Json: Data, _ response: ((T?, String?, String?) -> Void)?) {
        let tmp: T? = WolfModel.model(Json)
        response?(tmp, "", "0")
    }
    static func baseListFromJSON<T: WolfMapper>(_ Json: Data, _ response: (([T]?, String?, String?) -> Void)?) {
        let tmp: [T]? = WolfModel.modelList(Json)
        response?(tmp, "", "0")
    }
}
