...
class EditorViewController: UIViewController {
    ...
    fileprivate var mediaUpdated = [String:Int]()
    fileprivate var mediaUrlMap = [String:String]()
    ...
    fileprivate(set) lazy var titlePlaceholderLabel: UILabel = {
        let placeholderText = NSLocalizedString("添加标题", comment: "Post title placeholder")
...
//        let timer = Timer.scheduledTimer( timeInterval: 5, target: self, selector: #selector(runCode), userInfo: nil, repeats: true)
//        RunLoop.main.add(timer, forMode: RunLoopMode.commonModes)
       
    }
    @objc func runCode()
    {
        print( editorView.getHTML())
    }
    ...
    func onSubmit(_ sender: Any) {
        ...
        var content = self.editorView.getHTML();
        
        for (fileLocalPath,url) in mediaUrlMap
        {
            content = content.pregReplace(pattern: fileLocalPath, with: url)
        }
        mediaUrlMap=[:]
    ...
    override var keyCommands: [UIKeyCommand] {
        ...
        if richTextView.isFirstResponder {
            return [ UIKeyCommand(input:"B", modifierFlags: .command, action:#selector(toggleBold), discoverabilityTitle:NSLocalizedString("Bold", comment: "Discoverability title for bold formatting keyboard shortcut.")),
                     ...
                     UIKeyCommand(input:"U", modifierFlags: .command, action:#selector(EditorViewController.toggleUnderline(_:)), discoverabilityTitle: NSLocalizedString("Underline", comment:"Discoverability title for underline formatting keyboard shortcut.")),
...
extension EditorViewController : Aztec.FormatBarDelegate {
    ...
    func showLinkDialog(forURL url: URL?, text: String?, target: String?, range: NSRange, allowTextEdit: Bool = true) {
        ...
        alertController.addTextField(configurationHandler: { [weak self]textField in
            ...
            textField.addTarget(self,
                action:#selector(EditorViewController.alertTextFieldDidChange),
...
private extension EditorViewController
{
    ...
    func insertImage(_ image: UIImage) {
        ...
        if let imageData = UIImageJPEGRepresentation(image, 0.1)
        {
            uploadImage(fileName: imageID+".jpg", imageData: imageData) { (url) in
                if let _url = url
                {
                    self.mediaUpdated[imageID] = 1
                    self.mediaUrlMap[fileURL.absoluteString] = _url
                }
            }
        }
        
        Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(EditorViewController.timerFireMethod(_:)), userInfo: progress, repeats: true)
    }
    ...
    @objc func timerFireMethod(_ timer: Timer) {
        ...
        if mediaUpdated[imageId] == 1 || progress.completedUnitCount < 80
        {
...
extension FormattingIdentifier {
    ...
    private func gridicon(_ gridiconType: GridiconType) -> UIImage {
        let size = EditorViewController.Constants.formatBarIconSize
